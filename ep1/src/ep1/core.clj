...
(defn CreateReflectiveR [R PossibleReflectiveValues]
  (let [DataRemoveCondition (fn [x] (contains? (set R) x))
...
(defn MatrixMult [mat1 mat2]
  (let [Transpose #(apply map vector %)
        ...
        RowMult (fn [mat row] (map (partial DotProduct row) (Transpose mat)))
...
(defn TranformMatrixInBinaryMatrix [matrix]
  (let [ChangeOneValueToBinary #(if (> % 0) 1 0)
...
(defn MatrixOr [matrix_a matrix_b]
  (let [TwoBitOr #(if (or (= 1 %1) (= 1 %2)) 1 0)
...
(defn GetTransitiveR [n R Rn]
  (let [Rn_plus1 (TranformMatrixInBinaryMatrix (MatrixMult R Rn))]
    (if (= Rn_plus1 Rn)
      Rn
      (MatrixOr Rn (GetTransitiveR n R Rn_plus1)))))

(defn Transitive [n R]
  (let [R_binary (CreateBinaryMatrix n R)
