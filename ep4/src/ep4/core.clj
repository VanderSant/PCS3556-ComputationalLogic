...
(defn GetNumberOfVariablesByElement
  ...
  (let [all_symbols (GetAllPossibleSymbols grammar)
...
(defn GetNumberOfVariablesByIndex
  ...
  (let [rule (ep2/GetGramRule grammar index)
        elem (rule "value")]
...
(defn RemoveRedundantValues
  ...
  (let
   [RepeatedValuesMap (fn [rule pos]
    ...
    SymbolEqualValueFilter (fn [rule]
     ...
     (if (>= index number_of_symbols)
       ...
       (let
        [symbol_to_analyse (get all_possible_symbols index)
         ...
         UnitVariableTranformation (fn [element]
                                     (let
         ...
         OldElemMapFilter (fn [rule actual_index]
                            (let
...
(defn PerformeChomskyNormalization
  ...
  (let
   [[grammar_initial_value_corrected
     start_symbol_initial_value_corrected] (RemoveInitalSymbolFromRightSide grammar start_symbol end_symbols)
